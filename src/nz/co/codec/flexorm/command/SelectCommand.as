package nz.co.codec.flexorm.command
{
    ...
    {
		public var countMode:Boolean = false;
        ...
        override protected function prepareStatement():void
        {
            ...
            for (var table:String in _columns)
            {
                ...
                {
					if(!countMode) sql += mx.utils.StringUtil.substitute("t{0}.{1},", i, column);
                }
                i++;
                columnsAdded = true;
            }
            if (columnsAdded)
            {
				if(!countMode) sql = sql.substring(0, sql.length-1); // remove last comma
            }
            else
            {
				if(!countMode) sql += "*";
			...
			if(countMode) sql += "count(*) as count";
            sql += " from ";
            ...
            for (i = 0; i < len; i++)
            {
                sql += mx.utils.StringUtil.substitute("{0}.{1} t{2}", _schema, tables[i], i);
                if (i > 0)
                {
                    for (var fk:String in _joins[tables[i]])
                    {
                        sql += mx.utils.StringUtil.substitute(" on t{0}.{1}=t{2}.{3} and ", i-1, _joins[tables[i]][fk], i, fk);
                    }
                }
                if (i < len-1)
                {
            ...
            if (_filters.length > 0)
            {
                ...
                for each(var filter:IFilter in _filters)
                {
                    ...
                    {
                        sql += mx.utils.StringUtil.substitute("{0} and ",
                    ...
                    {
                        sql += mx.utils.StringUtil.substitute("t{0}.{1} and ",
            ...
            if (_sorts.length > 0)
            {
                ...
                {
                    sql += mx.utils.StringUtil.substitute("t{0}.{1} , ",
